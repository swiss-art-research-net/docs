...
As an example, we considered the image classification pipeline developed by SARI for the BSO project [GH link]. This pipeline aims at classifying images as being landscape paintings or not; thus the model can assign either the label `landscape` or the one `not landscape`. The pipeline is implemented as a set of Jupyter notebooks and consists of four steps:
